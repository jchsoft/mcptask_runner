...
module McptaskRunner
  ...
  class Installer
    ...
    # Writes config/waiting_strategy.yml (per-host override for short/long wait
    # durations) the first time the installer runs. Skipped when the file already
    # exists — the user edits it directly to retarget without re-running install.
    # Unlike configure_bug_destination this is OPT-IN: we copy the .example with
    # the recommended short waits so unattended hosts don't sleep 30 minutes
    # between retries, but operators can delete the file to revert to defaults.
    def configure_waiting_strategy
      puts '[Installer] Configuring waiting strategy (short/long wait durations)...'

      path = waiting_strategy_path

      if File.exist?(path)
        puts "[Installer] #{path} already exists — leaving untouched (delete to use defaults)"
        return
      end

      write_waiting_strategy_file(path)
      puts "[Installer] Waiting strategy written: #{path}"
    ...
    def write_waiting_strategy_file(path)
      FileUtils.mkdir_p(File.dirname(path))
      payload = {
