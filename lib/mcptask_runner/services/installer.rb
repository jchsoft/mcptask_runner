...
module McptaskRunner
  ...
  class Installer
    ...
    LOG_FILTER_SOURCE = File.expand_path('../log_filter.rb', __dir__).freeze
    ...
    def generate_launch_agent
      ...
      script = write_launcher_script(slug, chosen)
      write_plist(plist, label, slug)

      puts "[Installer] Launcher script written: #{script}"
    ...
    def write_plist(path, label, slug)
      FileUtils.mkdir_p(File.dirname(path))
      File.write(path, plist_content(label, slug))
    end

    def plist_content(label, slug)
      script    = xml_escape(launcher_script_path(slug))
    ...
    # ~/.claude/bin/mcptask-runner-<slug> — a named wrapper the LaunchAgent runs instead of
    # `bash -c "…"`, so macOS Login Items / Background app activity shows "mcptask-runner-<slug>"
    # (the display name is taken from ProgramArguments[0]) rather than a bare "bash".
    def launcher_script_path(slug)
      File.join(@helper_bin_dir, "mcptask-runner-#{slug}")
    end

    def write_launcher_script(slug, rake_task)
      path = launcher_script_path(slug)
      FileUtils.mkdir_p(File.dirname(path))
      File.write(path, launcher_script_body(rake_task, write_log_filter, slug))
      FileUtils.chmod(0o755, path)
      path
    end

    # ~/.claude/bin/mcptask-runner-log-filter.rb — copied from the gem's lib/ so the launcher
    # can invoke it with plain `ruby` (not `bundle exec`), keeping logging alive even while the
    # launcher is busy healing a broken/stale bundle.
    def write_log_filter
      path = File.join(@helper_bin_dir, 'mcptask-runner-log-filter.rb')
      FileUtils.mkdir_p(File.dirname(path))
      FileUtils.cp(LOG_FILTER_SOURCE, path)
      FileUtils.chmod(0o755, path)
      path
    end

    # Daily launcher body: keep deps fresh (a stale Gemfile.lock after git pull would otherwise
    # make `bundle exec` die), log which mcptask_runner version is now active so an update is
    # visible, and — when bundle actually installed something — remind the user to refresh this
    # LaunchAgent (its plist is generated once and does not self-update). All output — including
    # bundle install — is piped through the log filter, which timestamps entry-start lines and
    # rotates into a daily file per project (~/logs/mcptask_runner/<slug>/YYYY-MM-DD.log).
    def launcher_script_body(rake_task, filter_path, slug)
      ver = "grep -m1 'mcptask_runner (' Gemfile.lock"
      <<~SH
        #!/bin/bash -l
        # mcptask-runner daily launcher — generated by `rake mcptask_runner:install`. Do not edit by hand.
        set -o pipefail # so the script's exit status reflects the rake task, not the log filter
        {
          cd "#{@target_dir}" || { echo "[launcher] cannot cd to #{@target_dir}"; exit 1; }
          echo "[launcher] $(date '+%F %T') starting; active $(#{ver})"
          if bundle check; then
            echo '[launcher] dependencies up to date'
          else
            echo '[launcher] dependencies changed -> bundle install'
            bundle install \\
              && echo "[launcher] bundle install done; runner now $(#{ver})" \\
              && echo '[launcher] runner may have changed -> refresh the LaunchAgent: bundle exec rake mcptask_runner:install FORCE=1'
          fi
          bundle exec rake #{rake_task}
        } 2>&1 | ruby "#{filter_path}" "#{@log_base_dir}" "#{slug}"
      SH
    end
