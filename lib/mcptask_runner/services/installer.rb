...
module McptaskRunner
  ...
  class Installer
    ...
    def write_launcher_script(slug, rake_task)
      ...
      FileUtils.mkdir_p(File.dirname(path))
      File.write(path, launcher_script_body(rake_task, File.join(@log_base_dir, "#{slug}.log")))
    ...
    # bundle install — is redirected to the run log.
    def launcher_script_body(rake_task, log_path)
      # Both gem names: a host part-way through the migration to
      # mcptask-rails-runner still generates this launcher, and a pattern that
      # only knows the legacy name logs an empty version line there.
      ver = "grep -m1 -E 'mcptask[_-](rails-)?runner \\(' Gemfile.lock"
      <<~SH
        #!/bin/bash -l
